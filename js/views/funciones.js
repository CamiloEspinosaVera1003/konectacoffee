...
function marcar_tiposervicio(source){
    sw = 0;
    checkboxes=document.getElementsByTagName('input'); //obtenemos todos los controles del tipo Input
    for(i=0;i<checkboxes.length;i++){ //recoremos todos los controles
        
        let arr = checkboxes[i].name.split('-');
        
        if(checkboxes[i].name == "tiposervicio_todos"){ sw = 1; }
         
        if(checkboxes[i].type == "checkbox" && sw == 1 && arr[0] == 'Servicio'){ //solo si es un checkbox entramos y si es "TipoServicio_todos(sw==1)"
            checkboxes[i].checked=source.checked; //si es un checkbox le damos el valor del checkbox que lo llamo (Marcar/Desmarcar Todos)            
        }
    }
}
function marcar_TipoVehiculo(source){
    sw = 0;
    checkboxes=document.getElementsByTagName('input'); //obtenemos todos los controles del tipo Input
    for(i=0;i<checkboxes.length;i++){ //recoremos todos los controles
        ...
        if(checkboxes[i].name == "TipoVehiculo_todos"){ sw = 1; } 
        if(checkboxes[i].type == "checkbox" && sw == 1 && arr[0] == 'Vehiculo'){ //solo si es un checkbox entramos y si es "TipoVehiculo_todos(sw==1)"
            checkboxes[i].checked=source.checked; //si es un checkbox le damos el valor del checkbox que lo llamo (Marcar/Desmarcar Todos)            
        }
    }
}
...
function marcar_Tarifa(source){
    ...
    for(i=0;i<checkboxes.length;i++){ //recoremos todos los controles
        ...
        if(checkboxes[i].name == "Tarifa_todos"){ sw = 1; }
         
        if(checkboxes[i].type == "checkbox" && sw == 1 && arr[0] == 'Tarifa'){ //solo si es un checkbox entramos y si es "TipoServicio_todos(sw==1)"
            checkboxes[i].checked=source.checked; //si es un checkbox le damos el valor del checkbox que lo llamo (Marcar/Desmarcar Todos)            
        }
    }
}
...
//
function nombres_igual(nombre_buscar,campo_buscar,ruta_api,filter){
    var respuesta='*';
     
    $.ajax({        
        url: "ajax/ajxRequest2.php",
        data: { op: '69', nombre_buscar: nombre_buscar, campo_buscar: campo_buscar, ruta_api: ruta_api, filter_add:filter },
        dataType: 'json',
        type: 'POST',
        async: false,
        success: function(r) {           
...
$(document).on("keyup", ".inputNombres", function(){

    var temp=$(this).val()

    var temp2= temp.replace(/[^0-9a-zA-Z áéíóú]/g,'')

    var temp3=temp2.substr(0,50);
...
$(document).on("keyup", ".inputValores", function(){

    var temp=$(this).val()

    var temp2= temp.replace(/[^0-9]/g,'')

    var temp3=temp2.substr(0,12);

    $(this).val(  temp3 );

});

$(document).on("keyup", ".inputValorCorto", function(){

    var temp=$(this).val()

    var temp2= temp.replace(/[^0-9]/g,'')

    var temp3=temp2.substr(0,3);
...
$(document).on("keyup", ".inputTextArea", function(){
    ...
    var temp3=temp2.substr(0,80);

    $(this).val(  temp3 );

});

$(document).on("keyup", ".inputSoloLetras", function(){

    var temp=$(this).val()

    var temp2= temp.replace(/[^a-zA-Z]/g,'')
