...
var aDatosBoletas = new Array();
var aDatosEdad = new Array();
var aDatosEstatura = new Array();
var nRegistros;
                ...
                if(typeof r2.resultado.status === 'undefined'){
                    ...
                    $.each(r2.resultado, function(m, n) {
                       ...
                       var precio_comilla =  n.precio;
                        ...
                        var btnEditar = (n.estado == 'ACTIVO') ? '<a href="javascript:;"><img src="imagenes/edit.png" class="img-fluid title="Editar" onclick="upBoleta('+ n.id +','+ boleta_comilla +','+ descripcion_comilla +','+ precio_comilla +','+ n.categoriaEdad.id +','+ n.categoriaEstatura.id +');"></a>' : '<img src="imagenes/editar_no.png" class="img-fluid title="Editar">';
                            ...
                            if (r2.sts == 'OK') {//AQUI COMIENZA A PINTAR LA TABLA                
                                if(typeof r2.resultado.status === 'undefined'){
                                    ...
                                    $.each(r2.resultado, function(m, n) {
                                        ...
                                        console.log("GRupos:")
                                        console.log(n.privilegeGroupId)

                                          option += '<option value="' + n.privilegeGroupId + '">' + n.privilegeGroupName + '</option>';  
                                                        
                                    });				             
                                    
                                    $('#privilege_group_id').html(option); 

                                    $('#Update_privilege_group_id').html(option); 
...
function adicionarBoletas(nombre, descripcion, precio, idcategoriaEdad, idcategoriaEstatura, privilege_group_id, ext){
    let str_base64 = document.getElementById("img").src;
    //let imagen = str_base64.substring(23);
    let imagen=str_base64;
    //console.log(imagen);

    let respr = nombres_igual(nombre, 'nombre','api/boleteria/buscarTipoBoleta');

    console.log(aDatosBoletas);
    //alert("respr:"+respr);
   // return (false);
    ...
    if(respr == 0){
        ...
        $.ajax({        
            url: "ajax/ajxRequest2.php",
            data: { op: '32', nombre: nombre, descripcion:descripcion,precio:precio,idcategoriaEdad:idcategoriaEdad, idcategoriaEstatura:idcategoriaEstatura, imagen: imagen, extension: ext, privilege_group_id:privilege_group_id },
            dataType: 'json',
            type: 'POST',
            //async: false,
            success: function(r) { 
        ...
        alert('La boleta con nombre:'+nombre+', ya existe');
    } 
}

function actualizarBoletas2(id, nombre,descripcion,precio,idcategoriaEdad,idcategoriaEstatura, ext, base64, privilege_group_id){
    ...
    if(respr==0){
        ...
        $.ajax({        
            url: "ajax/ajxRequest2.php",
            data: { op: '35', id: id, nombre: nombre, descripcion:descripcion, precio:precio, idcategoriaEdad:idcategoriaEdad,idcategoriaEstatura:idcategoriaEstatura, ext:ext, base64:base64, privilege_group_id:privilege_group_id },
            dataType: 'json',
            type: 'POST',
            //async: false,
            success: function(r) { 
...
function abreModalActiDesactivaBoletas(){ 
    ...
    if(x == 0){
        ...
        if(x == 1){
            $("#p_cant").html("Est\u00E1 seguro de Activar/Desactivar "+x+" boletas?"); 
        }else{
            $("#p_cant").html("Est\u00E1 seguro de Activar/Desactivar "+x+" boletas?"); 
        }
...
function cargar_datos_privilege_group(){
    $.ajax({        
        url: "ajax/ajxRequest2.php",
        data: { op: '70' },
        dataType: 'json',
        ...
        success: function(r2) {            
            if (r2.sts == 'OK') {//AQUI COMIENZA A PINTAR LA TABLA                
                if(typeof r2.resultado.status === 'undefined'){
                    ...
                    var option = '<option value="0">Seleccione el Grupo de Privilegio</option>';
                    $.each(r2.resultado, function(m, n) {
...
function cargar_datos_condicion_edad(size,page){
    console.log('Cargar datos'); 
    $.ajax({        
        url: "ajax/ajxRequest2.php",
        data: { op: '37', page:page,size:size },
        dataType: 'json',
        type: 'POST',
        //async: false,
        success: function(r2) {            
            if (r2.sts == 'OK') {//AQUI COMIENZA A PINTAR LA TABLA                
                if(typeof r2.resultado.status === 'undefined'){
                    ...
                    $("#cant_pags2").html("Pag. "+r2.pag_consulta+" de: "+r2.cant_pags+"&nbsp;&nbsp;&nbsp;&nbsp;")
...
function cargar_datos_condicion_estatura(size,page){
    console.log('Cargar datos'); 
    $.ajax({        
        url: "ajax/ajxRequest2.php",
        data: { op: '44', size:size,page:page },
        dataType: 'json',
        type: 'POST',
        //async: false,
        success: function(r2) {            
            if (r2.sts == 'OK') {//AQUI COMIENZA A PINTAR LA TABLA                
                if(typeof r2.resultado.status === 'undefined'){
                    ...
                    //$('#condicion_mostrar').html(option); 
                    $("#cant_pags3").html("Pag. "+r2.pag_consulta+" de: "+r2.cant_pags+"&nbsp;&nbsp;&nbsp;&nbsp;")
...
function adicionarCEdad(nombre, edadMinima,edadMaxima){
        ...
        if(parseInt(edadMinima) < parseInt(edadMaxima)){
            ...
            let respr = nombres_igual(nombre, 'nombre','api/boleteria/buscarCategoriaEdad');
            ...
            if(respr==0){
                ...
                alert("La condicion de edad:"+nombre+" ya exixte!");
            }
...
function adicionarCEstatura(nombre, estaturaMinima,estaturaMaxima){
    ...
    if(parseInt(estaturaMinima) < parseInt(estaturaMaxima)){

        let respr = nombres_igual(nombre, 'nombre','api/boleteria/buscarCategoriaEstatura');
        ...
        $.ajax({        
            ...
            alert("La condicion de estatura:"+nombre+" ya exixte!")
        }

    }else{
        alert("La estatura inicial debe ser menor que la estaura final!");
            return(false);
    }
    //
...
function actualizarCEdad(id, nombre, edadInicial,edadFinal){
    ...
    if(parseInt(edadInicial) < parseInt(edadFinal)){
        ...
        $.ajax({        
           ...
           alert("La condicion de edad:"+nombre+" ya existe!");
        }
...
function actualizarCEstatura(id, nombre, estaturaInicial,estaturaFinal){
    ...
    if( parseInt(estaturaInicial) < parseInt(estaturaFinal) ){
        ...
        $.ajax({        
            ...
            alert("La categoria estatura:"+nombre+" ya existe!");
        }
